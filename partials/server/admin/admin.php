...
function validaLogin($usuario, $senha){
	...
	$query = "SELECT nome, usuario, senha FROM admin WHERE usuario = '$usuario' AND senha = '$senha'"; 	//Query que busca usuario e senha e compara 
	...
	if (mysql_num_rows($resultQuery) != 1){ 					//Verifica o resultado
		...
		$result = mysql_fetch_assoc($resultQuery);
			...
			echo "<meta http-equiv='refresh' content='0; url=http://localhost/area-do-cliente/#/formulario-envio'>";
		}
}	
...
//Função que protege a página verificando o usuario
function protegePaginaUsuario() {
	if($_SESSION['usuario'] != 'goliveira'){
		expulsaVisitante();
		exit;
	}
}
...
function expulsaVisitante() {
	...
	session_destroy();
	unset($_SESSION);
	echo "<meta http-equiv='refresh' content='0; url=http://localhost/area-do-cliente/#/envio'>";
}
